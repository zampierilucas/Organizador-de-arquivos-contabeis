		...
		for (int indexOfFiles = 0; indexOfFiles < source_files.length; indexOfFiles++) {
			...
			if (file_name.matches("(?i).*\\.(pdf|PDF|txt|TXT)$")) {
				...
				try {
					...
					if (TipeOfFile == null) {
						log.ErrorFound(logWrite, file_name, ParsedText, "Tipo de arquivo n�o encontrado");
						LOG_Error++;
						continue;
					}
					...
					if (CurrentDocDate.year() == null) {
						log.ErrorFound(logWrite, file_name, ParsedText, "Data do ducumento n�o encontrada");
						LOG_Error++;
						continue;
					}
					...
					if (CompanyCNPJ == null) {
						log.ErrorFound(logWrite, file_name, ParsedText, "CNPJ n�o encontrado no arquivo");
						LOG_Error++;
						continue;
					}
					...
					if (SRVCompanyPath == null) {
						log.ErrorFound(logWrite, file_name, ParsedText, "Caminho n�o encontrado no servidor");
						LOG_Error++;
						continue;
					}
					...
					// If path does not exists, create-it
						...
						// Log access error
							...
							log.ErrorFound(logWrite, file_name, ParsedText, "N�o foi possivel acessar o diret�rio");
							LOG_Error++;
							continue;
						}
					}
